...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (el) {
      const onWheel = (e: WheelEvent) => {
        ...
        // Prevents the page from scrolling vertically while we scroll the games horizontally
        e.preventDefault();
        
        el.scrollTo({
          left: el.scrollLeft + e.deltaY * 2.5, // Sensitivity multiplier
          behavior: 'auto' // 'smooth' can conflict with native snapping, 'auto' is usually better for wheel
        });
      };
  ...
  const handleLogin = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Updated credentials: Username 'nova', Password 'stacie2378'
    ...
    <div className="relative min-h-screen w-full flex flex-col bg-black overflow-hidden font-['Rajdhani']">
      ...
      <div className="relative z-10 flex flex-col h-screen max-h-screen">
        ...
        <main className="flex-grow flex flex-col justify-center pb-12">
          ...
          {/* Game Title Display - Increased height from h-40 to h-64 to prevent clipping */}
